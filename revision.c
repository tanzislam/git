...
static void paths_and_oids_clear(struct hashmap *map)
{
	...
	while ((entry = (struct path_and_oids_entry *)hashmap_iter_next(
			&iter))) {
				  ...
				  const struct object_id *oid)
{
	...
	if (!(entry = (struct path_and_oids_entry *)hashmap_get(map, &key,
								NULL))) {
	...
	for (left = i = 1; i < argc; i++) {
		...
		if (!seen_end_of_options && *arg == '-') {
			...
			opts = handle_revision_pseudo_opt(
				submodule, revs, argc - i, argv + i, &flags);
			if (opts > 0) {
				i += opts - 1;
				continue;
			}
...
static void explore_to_depth(struct rev_info *revs, uint32_t gen_cutoff)
{
	struct topo_walk_info *info = revs->topo_walk_info;
	struct commit *c;
