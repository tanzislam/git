...
static void name_rev(struct commit *commit, const char *tip_name,
		     timestamp_t taggerdate, int generation, int distance,
		...
		if (parent_number > 1) {
			...
			if (generation > 0)
				...
				new_name = xstrfmt("%.*s^%d", (int)len,
						   tip_name, parent_number);

			name_rev(parents->item, new_name, taggerdate, 0,
				 distance + MERGE_TRAVERSAL_WEIGHT, from_tag,
				 0);
		} else {
			name_rev(parents->item, tip_name, taggerdate,
				 generation + 1, distance + 1, from_tag, 0);
		}
	}
}
		    ...
		    void *cb_data)
{
	...
	if (o && o->type == OBJ_COMMIT) {
		...
		name_rev(commit, xstrdup(path), taggerdate, 0, 0, from_tag,
			 deref);
	}
	return 0;
}
