...
static void hmac(unsigned char *out, const char *key_in, size_t key_len,
			     ...
			     struct receive_hook_feed_state *feed_state)
{
	...
	if (feed_state->push_options) {
		...
		for (i = 0; i < feed_state->push_options->nr; i++)
			argv_array_pushf(
				&proc.env_array, "GIT_PUSH_OPTION_%d=%s", i,
...
static const char *push_to_deploy(unsigned char *sha1, struct argv_array *env,
...
static const char *push_to_checkout(unsigned char *hash, struct argv_array *env,
				    const char *work_tree)
{
	argv_array_pushf(env, "GIT_WORK_TREE=%s", absolute_path(work_tree));
	if (run_hook_le(env->argv, push_to_checkout_hook, hash_to_hex(hash),
			NULL))
