...
static int prune_worktree(const char *id, struct strbuf *reason)
{
	...
	if (!is_directory(git_path("worktrees/%s", id))) {
		strbuf_addf(reason,
			    _("Removing worktrees/%s: not a valid directory"),
			    id);
		return 1;
	}
	...
	if (stat(git_path("worktrees/%s/gitdir", id), &st)) {
		strbuf_addf(
			reason,
			_("Removing worktrees/%s: gitdir file does not exist"),
			id);
		return 1;
	}
	...
	if (read_result != len) {
		strbuf_addf(
			reason,
			_("Removing worktrees/%s: short read (expected %" PRIuMAX
			  " bytes, read %" PRIuMAX ")"),
	...
	if (!len) {
		strbuf_addf(reason,
			    _("Removing worktrees/%s: invalid gitdir file"),
			    id);
		free(path);
		return 1;
	}
	...
	if (!file_exists(path)) {
		...
		if (stat(git_path("worktrees/%s/index", id), &st) ||
			...
			strbuf_addf(
				reason,
				_("Removing worktrees/%s: gitdir file points to non-existent location"),
				id);
			return 1;
		} else {
			return 0;
			...
			const struct add_opts *opts)
{
	...
	if (!is_branch)
		...
		argv_array_pushl(&cp.args, "symbolic-ref", "HEAD", symref.buf,
				 NULL);
		if (opts->quiet)
...
static int list(int ac, const char **av, const char *prefix)
{
	...
	struct option options[] = { OPT_BOOL(0, "porcelain", &porcelain,
...
static void check_clean_worktree(struct worktree *wt, const char *original_path)
{
	...
	argv_array_pushf(&child_env, "%s=%s/.git", GIT_DIR_ENVIRONMENT,
			 wt->path);
