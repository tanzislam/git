	     ...
	     size_t idx_size, struct packed_git *p)
{
	...
	if (version == 1) {
		...
		unsigned long min_size =
			8 + 4 * 256 + nr * (hashsz + 4 + 4) + hashsz + hashsz;
...
static int open_packed_git_1(struct packed_git *p)
{
	...
	if (!p->index_data) {
		...
		for (m = the_repository->objects->multi_pack_index; m;
		     m = m->next) {
			 ...
			 const char *file_name, void *_data)
{
	...
	if (!strcmp(file_name, "multi-pack-index"))
		return;
	if (ends_with(file_name, ".idx") || ends_with(file_name, ".pack") ||
	    ends_with(file_name, ".bitmap") || ends_with(file_name, ".keep") ||
...
static off_t get_delta_base(struct packed_git *p, struct pack_window **w_curs,
...
get_delta_base_sha1(struct packed_git *p, struct pack_window **w_curs,
		    off_t curpos, enum object_type type, off_t delta_obj_offset)
{
		       ...
		       off_t obj_offset, struct object_info *oi)
{
	...
	if (oi->disk_sizep) {
		struct revindex_entry *revidx =
			find_pack_revindex(p, obj_offset);
	...
	if (oi->delta_base_sha1) {
		if (type == OBJ_OFS_DELTA || type == OBJ_REF_DELTA) {
			...
			base = get_delta_base_sha1(p, &w_curs, curpos, type,
						   obj_offset);
			if (!base) {
				type = OBJ_BAD;
				goto out;
		   ...
		   enum object_type *final_type, unsigned long *final_size)
{
	...
	while (delta_stack_nr) {
		...
		data = patch_delta(base, base_size, delta_data, delta_size,
				   &size);
...
const unsigned char *nth_packed_object_sha1(struct packed_git *p, uint32_t n)
{
