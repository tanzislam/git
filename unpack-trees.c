				  ...
				  const char *cmd)
{
	...
	msgs[ERROR_SPARSE_NOT_UPTODATE_FILE] = _(
		"Cannot update sparse checkout: the following entries are not up to date:\n%s");
	msgs[ERROR_WOULD_LOSE_ORPHANED_OVERWRITTEN] = _(
		"The following working tree files would be overwritten by sparse checkout update:\n%s");
	msgs[ERROR_WOULD_LOSE_ORPHANED_REMOVED] = _(
				 ...
				 struct unpack_trees_options *o)
{
	...
	if (was_skip_worktree && !ce_skip_worktree(ce)) {
		if (verify_absent_sparse(
			    ce, ERROR_WOULD_LOSE_UNTRACKED_OVERWRITTEN, o))
			return -1;
		ce->ce_flags |= CE_UPDATE;
	}
	return 0;
...
static int clear_ce_flags_dir(struct index_state *istate,
			      struct cache_entry **cache, int nr,
			      struct strbuf *prefix, char *basename,
			      ...
			      enum pattern_match_result default_match)
{
	...
	ret = path_matches_pattern_list(prefix->buf, prefix->len, basename,
					&dtype, pl, istate);
			    ...
			    enum pattern_match_result default_match)
{
	...
	while (cache != cache_end) {
		...
		if (slash) {
			...
			strbuf_addch(prefix, '/');
			cache += clear_ce_flags_1(istate, cache,
			  ...
			  int clear_mask, struct pattern_list *pl)
{
	...
	return clear_ce_flags_1(istate, istate->cache, istate->cache_nr,
				&prefix, select_mask, clear_mask, pl, 0);
}
		 ...
		 struct unpack_trees_options *o)
{
	...
	if (!o->skip_sparse_checkout) {
		 ...
		 * in loop #1 If the will have NEW_SKIP_WORKTREE, also set
